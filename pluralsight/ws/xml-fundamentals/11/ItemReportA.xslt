...
xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:data="urn:data">
	...
	<xsl:template match="price">
		<xsl:apply-templates select="$monetary-units">
				<xsl:with-param name="ele" select="."/>
		</xsl:apply-templates>
	</xsl:template>
	<xsl:template match="data:unit">
		<xsl:param name="ele"/>
		<xsl:if test="$ele[lang(current()/@xml:lang)]">
			<xsl:value-of select="concat(local-name($ele), ': ', $ele, ' ', @description, ' ')"/>
		</xsl:if>
	</xsl:template>
	<xsl:template match="length | width">
		<xsl:apply-templates select="$linear-units">
				<xsl:with-param name="ele" select="."/>
		</xsl:apply-templates>
	</xsl:template>
</xsl:stylesheet>

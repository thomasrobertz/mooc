...
public class DefaultOrderService implements OrderService {
   ...
   public List<Order> getOrderDetails(OrderStatus orderStatus, int fetchSize) {
      ...
      try {
         populateOrderDetails(orders, orderRepository.findByStatus(orderStatus.getCode(),
               new PageRequest(0, fetchSize)));
      } catch (Exception e) {
         log.error(
   ...
   public List<OrderItem> getOrderItems(long id) {
      ...
      try {
         ...
         log.error("An error occurred while retrieving order items for the order id |"
               + id + "|: " + e.getMessage(), e);
      }
      return orderItems;
   }
   ...
   private void populateOrderDetails(List<Order> orders,
      ...
      for (Iterator<OrderEntity> iterator = orderEntities.iterator(); iterator.hasNext();) {
         ...
         orders.add(new Order(entity.getId(), customer, entity.getOrderNumber(), entity
               .getTimeOrderPlaced(), entity.getLastUpdate(), OrderStatus
               .getOrderStatusByCode(entity.getStatus()).getDescription()));
      }
   }
   ...
   private void populateOrderItems(List<OrderItem> orderItems,
      ...
      for (Iterator<OrderItemEntity> iterator = orderItemEntities.iterator(); iterator
         ...
         CatalogItem catalogItem = new CatalogItem(catalogItemEntity.getId(),
               catalogItemEntity.getItemNumber(), catalogItemEntity.getItemName(),
               catalogItemEntity.getItemType());
         orderItems.add(new OrderItem(entity.getId(), catalogItem, entity.getStatus(),
               entity.getPrice(), entity.getLastUpdate(), entity.getQuantity()));
      }
   }
